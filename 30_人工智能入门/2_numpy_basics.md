...
形状 `(2, 4)` = 2 行 4 列，共 8 个元素。形状操作**不改变元素总数，只改变排列方式**。
...
**关键区别**：`reshape(4, 2)` 和 `.T` 结果形状都是 `(4, 2)`，但内容不同——reshape 是顺序重填，转置是行列互换。这是最容易混淆的点。

注意：这三个操作都**返回新结果，不修改原数组**（`arr` 本身不变）。

实际用途：深度学习里到处是 reshape，比如把图片 `(batch, 28, 28)` 展平成 `(batch, 784)` 送入全连接层，用的就是 `flatten` 的思想。
